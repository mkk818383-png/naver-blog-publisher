...
Phase 1: 네이버 블로그 5~10개 수집
    ...
    ▼
Phase 1-B: 핵심 정보 정리 → context.md (메뉴·가격·분위기·팁·출처 장부)
    │
    ▼
Phase 2~3: 출처 ID 정리 + 고정 구조로 다온로그 글 작성
    │
    ▼
Phase 4: 제목 후보 10개 생성 + 자동 검수
    │
    ▼
Phase 5: 검수 통과 후 HTML 컴파일 → 복붙용 post.html
```
...
| 모드 | 사용 시 |
|------|--------|
| **자동 리서치** | 필수 입력 + `[업체명]` — 에이전트가 네이버 블로그 검색·수집 |
| **수동 입력** | 필수 입력 + `[방문 메모]` — 크롤링 생략, 메모만으로 글 작성 |
...
posts/{가게이름}/
  ...
  titles.json        ← Phase 4: 1~10위 제목 후보와 각 선정 이유
